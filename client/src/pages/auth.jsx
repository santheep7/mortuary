...
export default function Auth() {
  ...
  const fetchClientByEmployeeId = async (employeeId) => {
    try {
      const res = await axios.get(`${API_BASE}/clients/employee/${employeeId}`);
      if (res.data && res.data.mortuary_logo) {
        setLoginClientLogo(res.data.mortuary_logo);
      } else {
        setLoginClientLogo(null);
      }
    } catch (error) {
  ...
  const fetchClientByClientId = async (clientId) => {
    try {
      const res = await axios.get(`${API_BASE}/clients/${clientId}`);
      if (res.data && res.data.mortuary_logo) {
        setRegisterClientLogo(res.data.mortuary_logo);
      } else {
